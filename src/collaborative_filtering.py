...
def collaborative_filtering(input_array, n_neighbors, ind, distance_measure='euclidean',
                             aggregator='mode'):
    """
    ...
    # Use input_array as both features and targets
    knn.fit(input_array, input_array)
    labels = knn.predict(input_array, True)
    # Find places where input-array has 0 values; this will give a 2-D array where
    # the rows are 2X1 arrays of indices (i, j)
    # indices = np.argwhere(input_array == 0)

    # MODIFIED: this method will now replace according to indices
    return_array = np.copy(input_array)
    return_array[ind] = labels[ind]
    return return_array
